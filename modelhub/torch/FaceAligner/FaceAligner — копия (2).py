...
class FaceAlignerNet(nn.Module):
    def __init__(self):
        ...
        d = _make_divisible(c16.out_ch*1.33, 8)
        self.fc11 = nn.Linear(c16.out_ch, d)
        self.fc12 = nn.Linear(c16.out_ch, d)
        
        self.fc2 = nn.Linear(d, d)
        self.fc2_act = nn.Hardswish()
        self.fc3 = nn.Linear(d, 4)
    ...
    def forward(self, inp):
        ...
        x = x.mean((-2,-1))
        x,y = self.fc11(x), self.fc12(x)
        
        
        x = torch.sqrt( F.leaky_relu(x)**2 + F.leaky_relu(y)**2 )
